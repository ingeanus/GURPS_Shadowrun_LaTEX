...
\begin{multicols}{3}
	...
	\subsubsection*{Alternative Wealth}\label{wealth}
	\begin{flushright}
		House Rule 
	\end{flushright}

	Some Shadowrunners tred a very strange line of simultaneously owning a fortune's worth of 'Ware, Telesma, Equipment, and so on — all the while being average or worse in terms of actual Wealth and being unable to leverage the usual benefits that such fortunes (read: the Wealth advantage) would provide them — better paying jobs, contacts and connections, higher Status, and so on. There are lots of hidden upsides to higher levels of the Wealth advantage that are not really relevant for many characters. After all, a player could seriously argue that they should receive better wages / shares from a run due to their higher Wealth than the other players!
	
	Such Wealth is purchased at half price and does not include \textit{any} societal benefits or negatives, up to an including: Status for high levels, improved wages for jobs, better connections, increases to any traits that scale off of your wealth (e.g. Independent Income), and so on. It also does not provide any plot protection like Signature Gear Does. This cash is still subject to the Settled and Wanderer restrictions — and the GM is advised not to allow players to get around Status and Cost of Living by simply being a Wanderer with lots of cash; enforce rules of travel housing, food, and environmental hazards!
	
	When taking this trait in addition to normal Wealth, one must buy Alternative Wealth as an additional positive/negative level to their current Wealth, indicating that they have either more or less funds than their Wealth would imply. Negative levels should be scrutinized particularly heavily and perhaps blanket disallowed.
	
	\textcolor{OliveGreen}{\textit{Example: Gandhi is trying to buy Comfortable Wealth, but also have enough cash to purchase the drones that he wants. The GM allows Wealth up to Wealthy, so he purchases Comfortable [10] + Alternative-Wealthy [5], for a total of [15] points. This provides him with all the social benefits of being Comfortable, but still gives $\times$5 starting money.}}
	
	\textcolor{OliveGreen}{\textit{Example: Swan is a creature of comfort that rolls with Wealthy [20], but the player wants them to have less assets due to a fall from grace that left them a Shadowrunner. The GM could easily just enforce a lower Wealth, but they allow a negative level of Alternative Wealth, coming out to Wealthy [20] - Alternative-Comfortable [-5] for [15] points. This provides the social benefits of Wealthy, but gives $\times$2 starting money.}}
	...
	An important consideration for the GM is whether Wealth provides its usual multiplication to Job Wages to Shadowrunning contracts. These missions are ostensibly still jobs, even if they are meant to be the main focus of the campaign rather than an off-screen way of gaining cash and background flavor. However, this can \textit{easily} derail negotiations and inter-party wages if not handled nicely. 
	
	The causes for these increased wages are due to the character's connections, influence, past history, and so on — so these should be taken into account by the Johnson / party's Fixer in negotiations in order to be fair. It might be fair to say that high levels of Wealth for a runner that affect jobs \textit{only} work for veteran runners that \textit{can} leverage their shadow connections and reputations for better wages. In such case, normal SINner Wealth is more of a flavor trait — or at best a heavily limited version of its normal self if the GM is not allowing runners to perform SINner style work during downtime.
	
	If these considerations are too annoying or troublesome, using the Alternative Wealth rules presented above still allow for higher levels of cash without having to deal with these issues. Regardless, if the GM does not allow many of the Basic Set's benefits to apply to a runner, they should consider reducing its cost or using the previously mentioned rules to be fair.
	...
	\subsubsection*{Large Size Modifier}
	\begin{flushright}
		-5 Points
	\end{flushright}

	All the normal features for SM, it just gives you points back.

	\subsubsection*{Small Size Modifier}
	\begin{flushright}
		+5 Points
	\end{flushright}
	
	All the normal features for SM, it just costs you points.
	
	\subsubsection*{Intermediate Size Modifier}\label{intermediate_sm}
	\begin{flushright}
		-1 Points
	\end{flushright}

	You are a metatype that is not \textit{quite} SM+1, but also too large to be comfortably called SM+0. You have \textit{some} of the benefits and drawbacks for both of these features, as listed below:
		
	\textit{Intermediate Size Creatures Have:}
	\begin{itemize}
		\itemsep 0pt
		\item +1 to Intimidation for SM
		\item +1 to Will versus Intimidation for SM
		\item +1 to hit with grapples
		\item Increased reach for SM+1 (B402)
		\item Counts as SM+1 for Pinning, giving +3 ST
		\item Higher SM for poison delay time
		\item +1 to rolls to notice them
		\item -1 to rolls to camouflage them
		\item SM+1 Equipment weights
		\item SM+1 Equipment costs
	\end{itemize}
	
	\textit{Intermediate Size Creatures Lack:}
	\begin{itemize}
		\itemsep 0pt
		\item -10\% cost to ST traits
		\item SM+1 for the purposes of determining if you can squeeze torso
		\item SM+1 for the purposes of immunity from constriction attacks
		\item +1 to be hit versus SM+0, due to relative SM
		\item -1 to hit versus SM+0, due to relative SM
		\item Increased Pressure Damage
		\item Increased Consumption or Life Support Needs\footnote{I don't include these in any Size Modifier trait. See the Behinds the Screen section for further explanations, but just take them yourselves if you want them.}
	\end{itemize}
	
	This trait can conceivably be applied to other intermediate sizes as well (e.g not quite SM+2, not quite SM+1), although that is left to the GM to balance. See \hyperref[behind_the_screen]{the Behind the Screen Section} for my thoughts behind how this trait was priced.
	...
	\textit{Cyberspace:} Covers information about how the matrix is entirely organized. Can be used for information on public hosts, grids, matrix architecture, people of important, etc. The area classes for Cyberspace are a bit different, but the GM can usually apply them similarly. Individuals can claim having 'Lived There' if they have Computer Operation at Skill Level 12+. Here are some example for inspiration (however the Basic Set rules are still incredibly useful regardless):
	...
	\begin{flushright}
		\textbf{10/H}
	\end{flushright}
	\textcolor{NavyBlue}{\textit{Defaults: Summoning-4}}
	...
	\paragraph{Ritual Spellcasting}\label{ritual_spellcasting_skill}
	\begin{flushright}
		\textbf{10/H}
	\end{flushright}
	\textcolor{NavyBlue}{\textit{Defaults: Spellcasting-4}}
	
	Ritual Spellcasting is a specific type of Spellcasting (See below) that makes use of distinct long-form methods to both reduce the strain of spellcasting alongside allow groups to partake in the process together.
	
	% TODO: Ritual Spellcasting 
	...
	\begin{flushright}
		\textbf{10/H}
	\end{flushright}
	\textcolor{NavyBlue}{\textit{Defaults: Compiling-4}}
...
\begin{multicols}{2}
	...
	\textbf{Advantages:} Combat Reflexes [15]; \hyperref[wealth]{Alternative-Wealthy} [10]
	...
	\textit{Choose [10] points from approriate Techniques, such as:}
	Combinations (MA80); Counterattack (H); Feint (H); Targeted Attack (Neck) (H); Targeted Attack (Skull) (H); Targeted Attack (Vitals) (H); Targeted Attack (Chinks) (H); Spinning Attack (H)
...
\end{coloredbox}
	...
	\subsubsection{Rules-Lite Planning and Flashbacks}
	
	One of the largest time-sinks when running Shadowrun, especially if your game leans Black-Trenchcoat, is the planning phase. For complex missions, there can be planning phases just to recon information so that you can plan the actual heist! This also contributes notably to the whole Decker-Pizza mentality, as much of this planning and reconnaissance can be done by a Decker without massive inputs from other runners — or at the very least it tends to be the least risky option, causing it to be the most commonly used route.
	
	A possible solution that gets thrown around a lot is a Blade in the Dark / pick your rule-lite-of-choice style Flashback system. The general gist of these (forgive me, I've never played Blades, only read up on it), is to allow players, when encountering a significant obstacle, to retro-causally justify some sort of gear, plan, or idea that might allow the team to avoid this. 
	
	As an example: the Infiltrator may be crawling through the vents when they come across a high speed filtration system meant to keep Gaseous Form Infected from sneaking in through the vents. They didn't plan for this and don't have equipment that might allow for easy bypass, so they use a Flashback, saying that they worked with the Decker to create a very short range EMP-style device that will fry some of the electronics in the fan motor after being attached to its case. The GM accepts this justification, calls for any appropriate rolls, and things continue.
	
	This has its positives and negatives, which largely appeal to different groups. It keeps the action flowing unimpeded, removes dependency on the player to be as competent as their Shadowrunner is, and prevents overplanning and paralysis. However, you still lose out of MacGyver like situations where the crew must improvise rapdily, it can short-cut surprises depending on how lenient the GM is, and it can sometimes cheapen the feeling of runs by making security seem too easy compared to the runners. Your mileage may vary with how important these are to you, however one ground truth is that \GURPS does not have an easy system built to allow this, so I'm providing some quick options for this section.
	
	The first and easiest is for the GM to simply allow such things to occur. No special traits are required, the GM simply allows players to justify some sort of reasonable explanation for how their characters would have prepared for this eventuality and then decides whether to allow the explanation; these Flashbacks may require a roll in order to succeed, perhaps requiring their own skill or perhaps being related to whatever justification the player provides, with better justifications removing penalties or providing bonuses to the roll. It is probably best for such traits to be limited in some way, similar to the Stress system in Blades. This could require existing resources, such as FP or HP depending on justifications, but it could also have its own resource pool that players could buy up (Exactly like FP).
	
	The more complicated option is to try and create such a system using existing \GURPS advantages of Serendipity and Gizmo, then allow the players to take these. This is invariably more expensive, but probably balanced to allow certain characters to do their shtick more explicitly while other may be planners and so on. The following are examples of that:
		
	\textbf{Major Flashback}
	\begin{flushright}
		21 Points
	\end{flushright}

	You are an incredibly skilled planner and are able to come up with useful or lucky solutions for a wide variety of possible problems even without requisite information. Whenever your character or another character that you could reasonably have had time to interact with and plan with in the recent past encounter a problem, you can attempt to use your Major Flashback advantage.
	
	To use this trait you must first come up with some \textit{reasonable} explanation for how your character previously planned for this possibility; examples include cases like having prepared a fake keycard beforehand, having lifted biometrics from the guy's house, having bribed the guard you run across beforehand, and so on. You do not have to \textit{actually} have done anything in relation to these events, it is just assumed that your character foresaw this situation and planned accordingly. This of course does mean that situations that are directly and \textit{completely} contradictory are impossible, however if there's a possibility it's okay! If your duffelbag was thoroughly searched, it's still possible (if unlikely) that the guards simply missed your elaborately hidden compartment — although an easier explanation (and modifier) would be that you simply bribed them beforehand! It is important to note that these justifications must be things \textit{you or your companions could have planned} — they cannot be as simple as the guard accidentally droping his key outside the bars!
	
	The GM must then assign a skill that best encapsulates the entire justification and a Task Difficulty to your explanation based on how \textit{difficult} it would be to pull off and on how \textit{complex} the explanation is. Additional modifiers are easily applicable, such as penalties for not having enough time to perform your explanation (maybe this was a rushed run), respective modifiers for the skill (maybe cultural familiarities to a social skill in a far-away country), and so on. Additionally, if the problem that you are encountering is \textit{not} an emergency, you have an additional -3 to the roll; emergencies are any difficulties that could likely lead to getting discovered, stopping the run in its tracks, threaten your character to the degree of causing a Fright Check or Major Wound, or cause similarly large difficulties.
	
	For example, bribing the guards to let you into the CEO's office unattended might be a -10 Fast-Talk roll even if you're Wealthy, but to let you in the back door during their break might be -3 or -4 — if you were running in Neo-Tokyo the GM might tack on the -3 cultural familiarity penalty! More examples could be, if the Infiltrator runs into a biometric lock on the MacGuffin's case, having previously broken into the high-rank CEO's home in order to grab his fingerprints might be a -8 Stealth roll due to the difficulty and complexity rivaling a small run, while lifting his fingerprints from his car in the parking lot could be a +2 Acting or Stealth roll — and perhaps if the run was on short-notice and the GM decides you did not have a lot of time to plan, you may have an additional -2.
	
	Once you know the modifier and skill they are free to choose whether to spend their ability and roll. If you do you must spend 1 Fatigue Point — your character spends their energy coming up with such a scheme, and presumably some others that will never see the light of day — and a successful skill roll causes your explanation to happen.
	
	The GM \textit{can} allow other players to make the roll if they decide that the explanation involves \textit{that player's character} performing more of the legwork or planning, but they are recommended to keep this to an absolute minimum if at all — the other character didn't pay for the advantage after all! If player do want to have a team-wide planning trait, the GM may allow them to split the cost of this trait amongst them all. If they do so, uses are shared between \textit{all} characters and can also only be used if all characters agree, although the rolling character is the one that spends FP.  
	
	You can use Major Flashback once per run/downtime segment \textit{or} per week (GM Decides when allowing the trait) each time you buy it. If the GM uses a weekly timeframe (the recommended choice), they may have additional levels divide the timeframe in half for each level instead.
	
	\textcolor{NavyBlue}{\textit{ Modifiers: Task Difficulty based on how difficult and complex the explanation is. Skill specific modifiers, such as Familiarity, Cultural Familiarity, Equipment Modifiers, Time Spent, and so on can apply. -3 if the problem isn't an emergency.}}

	\textit{\textcolor{OliveGreen}{Statistics: Serendipity (Game Time, +0\%; Wishing, +100\%; Aspected, Planning, -20\%; Costs Fatigue 1, -10\%; Full Power in Emergencies Only, -20\%; Requires Roll, -10\%) [21]}}
		
	\textbf{Minor Flashback}
	\begin{flushright}
		3 Points
	\end{flushright}

	This is a lesser version of the ability above, related only to equipment or tools that would be useful rather than a whole situation. Whenever \textit{your character} encounters a problem, you can attempt to use your Major Flashback advantage.
	
	To use this trait you must first come up with a \textit{reasonable} justification for some sort of equipment that your character might be able to use in order to overcome the problem. The item must also be something that fits in a coat pocket — or if you are stretching it, in a container that you have on your person, such as a bag. Justifications usually must fall into the normal Gizmo requirements: An item you own but where not carrying, an item that you \textit{probably} could have owned, or an inexpensive item widely owned. You can \textit{always} attempt to justify and roll for items that do not fit any of those criteria (e.g. having a Biometric sample for the door you ran across), but all such cases have an additional -5 modifier to the rolls below and may suffer harsher than normal penalties — however this lets you pull extremely difficult to foresee or acquire equipment out of your ass! This equipment does not generally enter play until this trait is used, meaning it cannot be found by scanners, searches, damaged by attacks, or so on.
	
	The GM might also allow a fourth category, being an expensive item you could reasonably get your hands on (e.g. through a contact), likely have to give a favor for and which must be return,  — failure to do so should have steeper consequences than simply paying the cost for the item! In general, the GM should always be a little wary of allowing this trait to shortcut rules for getting equipment and \textit{probably} not allow paying to keep the equipment for the fourth case.
	
	This \textit{can technically} allow a player to acquire certain pieces of 'ware, although the GM is heavily advised to require players to still pay for the 'ware with points and essence and is free to ban such cases in the name of expediency and simplicity. 
	
	The GM must then assign a skill that best encapsulates the entire justification and a Task Difficulty to your explanation based on how \textit{difficult} the item might be to have gotten this far and on how \textit{likely} it is for your character to have access to it. Additional modifiers are easily applicable, such as penalties for not having enough time to acquire such a device beforehand. This could be a +4 IQ or Guns roll to find your pistol in your pocket when the Johnson meet gets ambushed, perhaps a -4 Forgery roll to have used your Forgery Kit to have made a fake keycard for the door, or a Diplomacy-2 roll to acquire Gecko Gear beforehand from your contact. Additionally, if the problem that you are encountering is \textit{not} an emergency, you have an additional -3 to the roll; emergencies are any difficulties that could likely lead to getting discovered, stopping the run in its tracks, threaten your character to the degree of causing a Fright Check or Major Wound, or cause similarly large difficulties.
	
	Once you know the modifier and skill they are free to choose whether to spend their ability and roll. If you do, a successful skill roll allows you to bring the equipment into play and use it.
	
	You can use Minor Flashback once per run/downtime segment \textit{or} per week (GM Decides when allowing the trait) each time you buy it. If the GM uses a weekly timeframe (the recommended choice), they may have additional levels divide the timeframe in half for each level instead.

	\textit{\textcolor{OliveGreen}{Statistics: Gizmo (Cosmic, Avoiding Drawbacks (Secondary, x1/5\footnote{Shamelessly stolen from Affliction. It could be built with Limited By Margin, but since I don't want to have varying levels of Cosmic effects based on your Margin (e.g. What is 2/5 of Cosmic?), I think it's fair to cut that out and make it 1/5.}), +10\%; Game Time, +0\%; Aspected, Planning, -20\%; Full Power in Emergencies Only, -20\%; Requires Roll, -10\%) [3]}}
