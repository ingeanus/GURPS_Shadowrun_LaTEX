\begin{section_header}[gray]
	\textbf{\section{Riggers}\label{riggers}}
\end{section_header}
...
\begin{multicols*}{3}
	...
	While jumped in, the rigger benefits from a Talent level equal to his Control Rig rating. This provides a bonus to all rolls to "rig" well, which include control, piloting, sensor, mounted weaponry rolls, and jumping in rolls, among others.
...
%	TODO: Come back and update mentions for Meta-Tech
	...
	Even expanding the ally costs, as noted under \GURPS Social Engineering p42, does not help this very much, especially considering that bodies and pilot programs should be bought separately in the first place! Instead, we use the \GURPS Meta-Tech rules for Constructs, allowing us to build the drones as characters and then convert that to money. Rules for this can be found in that source book, as the statistics are already determine and written in their respective statblocks. Pre-included Pilot Programs are priced according to this, but upgrades are priced via fiat based on Software Costs, Ultratech AI rules, and general points values as there is simply no great way to include do such things otherwise.
