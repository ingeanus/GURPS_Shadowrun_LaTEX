...
\begin{multicols*}{2}
	...
	Let's perform the math like before: There are 267 IQ skills, meaning a Large Talent is 5.6\%, which naively gives IQ! (Accessibility, Large Talent, -5\%) [9.5]. If we apply Zipf's Law we get 22.5\% for IQ! (Accessibility, Large Talent, -30\%) [7]. This is getting close to \textit{halve the effectiveness}. There is a strong argument that people are more likely to specialize harder for IQ skills, however this is actually an argument \textit{for this issue}, as it simply means that we should discount IQ! further because Talents are even better at picking one specialization, which is all you ever want.
	
	For this reason, IQ! is [15] and disconnected from Per and Will.
