\begin{section_header}[brown]
	\textbf{\section{Critters}}
\end{section_header}
...
\begin{multicols*}{3}
	...
	\begin{center}
		...
		\begin{tabular}{|c|c|c|}
			\hline
			Limitation & Percentage & Cost\\
			\hline
			Subterranean Creatures & -65\% & 18/43/68 \\
			\hline
		\end{tabular}
		}
		\end{adjustwidth}
	...
	\subsubsection{Divination}\label{divination}
	\begin{flushright}
		28 Points
	\end{flushright}

	The critter has the ability to divine answer regarding the future by observing the astral plane and the effects the physical world has upon it. Whether or not this is true divination or something more akin to Astral Stastical Modelling is a somewhat undecided fact, but it nevertheless does allow for somewhat accurate predictions.
	
	To perform a divination, the critter must have either some material link or be able to see the aura of a target. This need not be a physical person, it can also be events or ideas, but those still require some sort of link, which can be seeing the aura of a location, having an item from an event, or perhaps having an object related to the idea. Performing the divination requires spending around 2 1/2 hours performing auguries, sortileges, or similar tasks depending on tradition or the critter. During this time, their concentration can be broken as per normal Concentration rules, ruining the attempt.
	
	This process takes 2 FP (Seperate from any fatigue lost from the event itself), ending with the critter rolling against IQ-2, Thaumatology-2, or some similar Hard/Very Hard skill as appropriate. Success provides a useful divination based off the Margin of Success, ranging from a cryptic answer amounting to the most likely outcome all the way up to an explicit answer to the question. Failure provides nothing useful, often a result or answer too cryptic to be helpful while Critical Failure permanently blocks out that option for all future premonitions!

	\textcolor{OliveGreen}{\textit{Statistics: Precognition (Directed, +100\%; Reliable 6, +30\% Active Only, -60\%; Magical, -10\%; Takes Extra Time 4\footnote{While \GURPS usually recommends limiting this to 2 levels and taking Preparation Required, that is a much bigger discount for such a small timeframe that I am using this.}, 160 minutes, -40\%; Trigger, Uncommon OR Vision-Based, -10\%\footnote{\textcolor{Blue}{\href{http://forums.sjgames.com/showpost.php?p=2006453&postcount=14}{See here for discussion on pricing. If you use this method I recommend capping A+B to 1.}}}) [27.5] }}
	...
	\subsubsection{Natural Weapon}\label{natural_weapon}
	\begin{flushright}
		Var. Points
	\end{flushright}

	Many critter have natural weapons of some kind beyond normal human capabilities. While these are rarely ever magical to any real degree, they are listed here for ease of use compared to the original Shadowrun.
	
	These often take the form of the Striker, Claws, Hooves, Teeth, or similar related advantages from the Basic Set.

	\subsubsection{Noxious Breath}\label{noxious_breath}
	\begin{flushright}
		16.1 Points + 6.9 Points per Level\footnote{Uses Kromm's home rule that further levels of Affliction cost 3 points base.}
	\end{flushright}
	
	The critter is able to emit some form of toxic aerosol that causes nausea or worse when breathed by other creatures.
	
	To do so, it must make an Attack maneuver, targeting a cone 10 yards long and 3 yards wide at its end. Creatures caught inside who breath in the attack must roll HT + 1 - the Critter's Magic. Success prevents any effects, while failure causes Nausea for a number of minutes equal to Margin of Failure; failure by 5 or more causes \textit{Retching} instead for a number of minutes equal to Margin of Failure, after which they are stunned until they can succeed on an HT roll (once per second). Creatures over 5 yards away who are hit gain a +3 to their HT rolls to resist.
	
	Higher Magic imposes penalties as described above.
	
	\textcolor{OliveGreen}{\textit{Statistics: Affliction (Nauseated, +30\%; Side Effect, Retching +10\%; Cone, +70\%; Increased 1/2D Range, $\times$5, +10\%; Respiratory Agent, +50\%; Magical, -10\%; Reduced Range, 1/10, -30\%) [23] }}
	...
	\subsubsection{Psychokinesis}\label{psychokinesis}
	\begin{flushright}
		x Points 
	\end{flushright}
	...
	\begin{center}
		...
		\begin{tabular}{|c|c|}
			\hline
			Metatype & Ghoul Point Cost \\
			\hline
			Human, Dwarf, Elf & 53 \\
			Orc, Nartaki & 49 \\
			Hanuman, Ogre, Koborokuru  & 49 \\
			Wakyambi & 53 \\
			Cyclopean & 46 \\
			Trolls, Minotaur & 48 \\
			Giant, Sasquatch & 46 \\
	...
	% TODO: At a later time, since these are relatively low priority for most games.
